...
public class GameBoard
{
	...
	public void updatedBoard(int x, int y, BoardComponent boardComponent){
		board.get(x).set(y,boardComponent);
	}
	...
	public void updatedBoard(BoardComponent bc1, BoardComponent bc2) {

		for(int i = 0 ; i<board.size(); ++i){
			ArrayList<BoardComponent> row = board.get(i);
			for(int j=0; j< row.size();++j){
				if(bc1 == row.get(j)){
					board.get(i).set(j,bc2);
				}
			}
		}
